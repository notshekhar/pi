...
api.commands.override("cost", { description: "...", handler: (ctx) => {/* ... */} });
```
...
api.providers.register({
  ...
  models: [
    { id: "fast-1", name: "Fast 1", contextWindow: 128000, maxOutput: 16000,
...
api.providers.register({
  id: "myvendor",
  auth: { mode: "oauth" },
  async getModel(modelId, ctx) {           // ctx: { apiKey, fetch }
...
api.turn.use({
  ...
  onBeforeTurn(ctx) {           // ctx: { input, cwd, sessionId, agent, modelId }
...
```jsonc
// package.json
{ "name": "loop-ext-demo", "version": "1.0.0", "type": "module",
